...
fn verify_prehash_unchecked(
    ...
    let signature =
        Signature::<NistP256>::from_slice(sig_rs).map_err(|_| VerifyError::InvalidSignature)?;
    ...
    verify_prehashed::<NistP256>(&q, &z, &signature).map_err(|_| VerifyError::SignatureInvalid)
}
