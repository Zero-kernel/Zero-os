...
// R148-I9 FIX: SHA-256 implementation moved to shared `crypto` crate.
// Both audit and livepatch now use the same canonical FIPS 180-4 code path.
use kernel_crypto::sha256;

/// SHA-256 block size in bytes (for HMAC padding)
const SHA256_BLOCK_SIZE: usize = sha256::BLOCK_SIZE;
...
/// R148-I9 FIX: SHA-256 type alias to the shared `crypto` crate implementation.
/// All audit hash-chain, HMAC-SHA256, and FIPS KAT code paths now use the same
/// canonical implementation as the livepatch signature verifier.
