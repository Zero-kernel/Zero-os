...
pub fn demo_basic_syscalls() {
    ...
    // 演示1: getpid系统调用
    klog!(Info, "1. Testing sys_getpid...");
    let pid = syscall_dispatcher(SyscallNumber::GetPid as u64, 0, 0, 0, 0, 0, 0);
    if pid >= 0 {
        klog!(Info, "   ✓ Current PID: {}", pid);
    } else {
        klog!(Error, "   ✗ Failed with error code: {}", pid);
    }
    ...
    if ppid >= 0 {
        klog!(Info, "   ✓ Parent PID: {}", ppid);
    } else {
        klog!(Error, "   ✗ Failed with error code: {}", ppid);
    }
    ...
    if result >= 0 {
        klog!(Info, "   ✓ Wrote {} bytes", result);
    } else {
        klog!(Error, "   ✗ Failed with error code: {}", result);
    }
...
pub fn demo_process_syscalls() {
    ...
    // 演示1: fork系统调用
    klog!(Info, "1. Testing sys_fork...");
    let child_pid = syscall_dispatcher(SyscallNumber::Fork as u64, 0, 0, 0, 0, 0, 0);
    if child_pid >= 0 {
        klog!(Info, "   ✓ Forked child process with PID: {}", child_pid);
    } else {
        klog!(Error, "   ✗ Fork failed with error code: {}", child_pid);
    }
    ...
    if result >= 0 {
        klog!(Info, "   ✓ Yielded CPU successfully");
    } else {
        klog!(Error, "   ✗ Yield failed with error code: {}", result);
    }
...
pub fn demo_error_handling() {
    ...
    // 演示1: 无效的文件描述符
    klog!(Info, "1. Testing invalid file descriptor...");
    let msg = b"test";
    let result = syscall_dispatcher(
        SyscallNumber::Write as u64,
        999, // 无效的fd
...
pub fn demo_syscall_performance() {
    ...
    klog!(Info, "1. Benchmarking getpid (lightweight syscall)...");
    let iterations = 1000;
